...
1. Runs `cmd/generate/main.go` (a thin CLI over `internal/codegen`, `internal/dockergen`, `internal/mdgen`) repeatedly with different `-type` flags to emit gateway-specific artifacts that `oapi-codegen` cannot produce: `providers/client/config.go`, `providers/constants/constants.go`, `providers/transformers/*.go`, `providers/registry/registry_data.go`, `config/config.go`, `Configurations.md`, and every `examples/docker-compose/*/.env.example`. Generated files contain only spec-driven content; the static runtime code lives in hand-written sibling files in the same packages: `providers/client/client.go` (`Client` interface, `ClientImpl`, `NewHTTPClient`), `providers/registry/registry.go` (`ProviderConfig`, `ProviderRegistry`, `BuildProvider`), `config/load.go` (`Load`, `String`), and `providers/constants/static.go` (`AuthType*` consts, `ListModelsTransformer`). Edit those directly - never the generated siblings.
...
`config/config.go` is generated (structs only; `Load`/`String` live in hand-written `config/load.go`) - every supported env var lives in struct tags there and is mirrored into `Configurations.md`. Link users to `Configurations.md` rather than enumerating vars in prose; it'll go stale.
...
- **No magic numbers or magic strings** - never hardcode literal numbers or strings that carry meaning (phases, actions, fail modes, limits, status codes, header names, etc.). Name them as consts (grouped `const (...)` blocks like the `Action*` / `Phase*` / `FailMode*` sets in `internal/guardrails`) and reference the const everywhere, including tests.

## Release Automation

**RELEASES ARE AUTOMATED:** Every inference-gateway repo releases via semantic-release: the version, changelog, tag, GitHub release, and package publish all derive from Conventional Commit messages once a PR merges. NEVER, in any repo:
...
Land a correctly typed Conventional Commit and let the pipeline release. If a task seems to require a manual release step, say so in your output instead of doing it.

## Security & Configuration Tips

Do not commit provider API keys, tokens, or local `.env` files. Use the checked-in `.env.example` files as templates. Review authentication, telemetry, and routing changes carefully because they affect gateway-wide behavior.
