...
| VISION_ENABLED               | `false`       | Enable vision/multimodal handling for all providers. When enabled, image content is stripped from requests to models not known to accept images and passed through to vision-capable models. When disabled, image content is forwarded to the provider untouched                                                                                                          |
...
| AUDIO_ENABLED                | `false`       | Enable the Audio API (POST /v1/audio/speech). When disabled, the endpoint returns a 404. Served by providers with speech support (currently openai) or by the local llama-tts engine                                                                                                                                                                                      |
