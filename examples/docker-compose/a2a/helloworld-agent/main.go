...
func main() {
	...
	greetingService := NewMockGreetingService(logger)
	greetingToolHandler := NewGreetingToolHandler(greetingService, logger)
	greetingToolProvider := NewGreetingToolProvider(greetingToolHandler)

	toolsHandler := adk.NewToolsHandler(logger, greetingToolProvider)

	agent := adk.NewA2AAgent(cfg, logger, client, toolsHandler)

	greetingTaskProcessor := NewGreetingTaskResultProcessor(logger)
	greetingInfoProvider := NewGreetingAgentInfoProvider(logger)

	agent.SetTaskResultProcessor(greetingTaskProcessor)
	agent.SetAgentInfoProvider(greetingInfoProvider)
